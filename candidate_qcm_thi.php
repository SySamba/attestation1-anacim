...
<body class="bg-light">
        ...
        <?php if ($completed_session): ?>
            ...
            <div class="row justify-content-center">
                <div class="col-md-8">
                    <div class="row">
                        <div class="col-md-6">
                            <h4 class="text-primary mb-0">Test QCM - <?php echo $epreuve; ?></h4>
    ...
    <script>
        ...
        function startTest() {
            testInProgress = true; // Activer la protection beforeunload
            fetch('qcm_api.php', {
                method: 'POST',
                headers: {
        ...
        function submitTest() {
            if (confirm('Êtes-vous sûr de vouloir terminer le test ? Cette action est irréversible.')) {
                testInProgress = false; // Désactiver la protection beforeunload
                clearInterval(timerInterval);
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        // Redirection immédiate vers les résultats sans alerte
                        testInProgress = false; // S'assurer que beforeunload est désactivé
                        if (data.redirect_url) {
                            window.location.href = data.redirect_url;
                        } else {
                            window.location.href = 'candidate_qcm_results.php?phase=<?php echo $phase; ?>&epreuve=<?php echo $epreuve; ?>';
                        }
                    } else {
                        alert('Erreur: ' + data.message);
                        // Restore button state
                        if (submitButton) {
                            submitButton.innerHTML = '<i class="fas fa-check me-2"></i>Terminer le Test';
        ...
        // Prevent page refresh/close during test ONLY
        let testInProgress = false;
        
        window.addEventListener('beforeunload', function(e) {
            if (currentSession && timerInterval && testInProgress) {
                e.preventDefault();
